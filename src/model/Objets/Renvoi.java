...
public class Renvoi extends Objet {

    public Renvoi() {
        super();
        this.name = "Renvoi";
        this.taux = 2;
        this.description = "Objet permettant de faire retourner un pion adverse proche de sa base si un de ses pions est à côté de l'un de vos pions";
    }
    ...
    public void setPlateauObjet(PlateauJeu pan, Plateau p, InGameView game) {
        game.textTour.setText("Selectionnez le pion adverse à renvoyer ");
                            ...
                            public void mouseClicked(MouseEvent e) {
                                ...
                                if (pan.lastPion != null) {
                                    if(!p.plat_jeu.get(c).pion.joueur.equals(pan.lastPion.joueur)) {
                                        if(p.isAdjacent(c,pan.lastPion.coordonnee)) {
                                            Coordonnee base = getBaseJoueur(p.plat_jeu.get(c).pion.joueur, p);
                                            Pion pion = p.plat_jeu.get(c).pion;
                                            p.plat_jeu.get(c).pion = null;
                                            LinkedList<Coordonnee> tmp = Plateau.getRandNextCoord(base);
                                            for(int i=0; i<tmp.size();i++) {
                                                if(p.plat_jeu.get(tmp.get(i)).pion==null && p.plat_jeu.get(tmp.get(i)).lockedTurn==0) {
                                                    pion.coordonnee = tmp.get(i);
                                                    p.plat_jeu.get(tmp.get(i)).pion = pion;
                                                    pan.setPlateauView();
                                                    pan.container.repaint();
                                                    pan.container.revalidate();
                                                    game.revalidate();
                                                    game.repaint();
                                                    pan.joueurenjeu.sacRemoveItem(new Renvoi());
                                                    // on vérifie si le pion est sur une case avec un objet dessus
                                                    if (p.plat_jeu.get(tmp.get(i)).obj != null) {
                                                        // on ajoute l'objet
                                                        pan.lastPion.joueur.sac.add(p.plat_jeu.get(tmp.get(i)).obj);
                                                        p.plat_jeu.get(tmp.get(i)).obj = null;
                                                    }
    ...
    public Coordonnee getBaseJoueur(Joueur j, Plateau p) {
        for (Coordonnee c : p.plat_jeu.keySet()) {
            if (p.plat_jeu.get(c).hasTuile()) {
                if (p.plat_jeu.get(c).getTuile().getNbBase() == j.getTurn()) {
                    return c;
                }
            }
        }
